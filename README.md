...
- All your project code into a single main package (`sfake/`)
...
```bash
<root directory>
├── sfake/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `sfake`
...
2. Clone the `sfake` repository

3. Open a shell into your local `sfake` directory and run 
    ```bash
    ./rename-template.sh sfake
    ``` 
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `sfake`, in this example)
...
13. Put your main (resp. test) code in `sfake/` (resp. `test/`)

## How to do stuff

### Run your code as an application

This will execute the file `sfake/__main__.py`:
```bash
python -m sfake 
```
...
GitHub actions automatically release a new version of `sfake` on PyPi whenever commits are pushed on either the `main`/`master` or `develop` branches, as well as when new tags are pushed.
